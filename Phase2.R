...
demo <- demo %>% distinct(RID,.keep_all = T)
demo <- demo[,-1]
base_v1 <- base[,c(1,10:ncol(base))]
base_v1 <- summaryBy(.~RID, FUN = quantile,na.rm = T,data = base_v1)
base_v1 <- cbind(demo,base_v1)
base_v1 <- base_v1[,-9]
base_v1 <- base_v1[!is.na(base_v1$Diagnosis),]
base_v1$Diagnosis<- as.factor(base_v1$Diagnosis)

#velocity dataframe
vel_v1 <- vel[,c(1,10:ncol(vel))]
vel_v1 <- summaryBy(.~RID, FUN = quantile,na.rm = T,data = vel_v1)
vel_v1 <- cbind(demo,vel_v1)
vel_v1 <- vel_v1[,-9]
vel_v1 <- vel_v1[!is.na(vel_v1$Diagnosis),]
vel_v1$Diagnosis<- as.factor(vel_v1$Diagnosis)

#accel dataframe
accel_v1 <- accel[,c(1,10:ncol(accel))]
accel_v1 <- summaryBy(.~ RID, FUN = quantile,na.rm = T, data = accel_v1)
accel_v1 <- cbind(demo,accel_v1)
accel_v1 <- accel_v1[,-9]
...
#abs_accel dataframe
abs_accel_v1 <- abs_accel[,c(1,10:ncol(abs_accel))]
abs_accel_v1 <- summaryBy(.~RID,FUN =quantile,na.rm = T,data = abs_accel_v1)
abs_accel_v1 <- cbind(demo,abs_accel_v1)
abs_accel_v1 <- abs_accel_v1[,-9]
abs_accel_v1 <- abs_accel_v1[!is.na(abs_accel_v1$Diagnosis),]
abs_accel_v1$Diagnosis<- as.factor(abs_accel_v1$Diagnosis)
abs_accel_v1 <- abs_accel_v1[,1:849]
...
table(base_train_data$Diagnosis)

vel_mvsd_train_data<- vel_train_data[vel_train_data$Diagnosis !='CN',]
vel_cn_train_data <- vel_train_data[vel_train_data$Diagnosis == 'CN',]
vel_mvsd_train_data$Diagnosis <- factor(vel_mvsd_train_data$Diagnosis)
vel_train_data <- SMOTE(Diagnosis ~., data= vel_mvsd_train_data, k=5, perc.over = 90)  
vel_train_data <- rbind(vel_train_data,vel_cn_train_data)


accel_mvsd_train_data<- accel_train_data[accel_train_data$Diagnosis !='CN',]
accel_cn_train_data <- accel_train_data[accel_train_data$Diagnosis == 'CN',]
accel_mvsd_train_data$Diagnosis <- factor(accel_mvsd_train_data$Diagnosis)
accel_train_data <- SMOTE(Diagnosis ~., data= accel_mvsd_train_data, k=5, perc.over = 90)  
accel_train_data <- rbind(accel_train_data,accel_cn_train_data)


abs_accel_mvsd_train_data<- abs_accel_train_data[abs_accel_train_data$Diagnosis !='CN',]
abs_accel_cn_train_data <- abs_accel_train_data[abs_accel_train_data$Diagnosis == 'CN',]
abs_accel_mvsd_train_data$Diagnosis <- factor(abs_accel_mvsd_train_data$Diagnosis)
abs_accel_train_data <- SMOTE(Diagnosis ~., data= abs_accel_mvsd_train_data, k=5, perc.over = 90)  
abs_accel_train_data <- rbind(abs_accel_train_data,abs_accel_cn_train_data)
...
base_gbm_avg_sens <- (base_bl_gbm_results[[4]][1]+base_bl_gbm_results[[4]][2]+
                        base_bl_gbm_results[[4]][3])/3

base_gbm_avg_spec <- (base_bl_gbm_results[[4]][4]+base_bl_gbm_results[[4]][5]+
                        base_bl_gbm_results[[4]][6])/3
...
base_rf_avg_sens <- (base_rf_results[[4]][1]+base_rf_results[[4]][2]+
                       base_rf_results[[4]][3])/3

base_rf_avg_spec <- (base_rf_results[[4]][4]+base_rf_results[[4]][5]+
                       base_rf_results[[4]][6])/3
...
base_svm_avg_sens <- (base_svmresults[[4]][1]+base_svmresults[[4]][2]+
                        base_svmresults[[4]][3])/3

base_svm_avg_spec <- (base_svmresults[[4]][4]+base_svmresults[[4]][5]+
                        base_svmresults[[4]][6])/3
...
base_knn_avg_sens <- (base_knn_results[[4]][1]+base_knn_results[[4]][2]+
                        base_knn_results[[4]][3])/3

base_knn_avg_spec <- (base_knn_results[[4]][4]+base_knn_results[[4]][5]+
                        base_knn_results[[4]][6])/3
...
vel_gbm_avg_sens <- (vel_bl_gbm_results[[4]][1]+vel_bl_gbm_results[[4]][2]+
                       vel_bl_gbm_results[[4]][3])/3

vel_gbm_avg_spec <- (vel_bl_gbm_results[[4]][4]+vel_bl_gbm_results[[4]][5]+
                       vel_bl_gbm_results[[4]][6])/3
...
vel_rf_avg_sens <- (vel_rf_results[[4]][1]+vel_rf_results[[4]][2]+
                      vel_rf_results[[4]][3])/3

vel_rf_avg_spec <- (vel_rf_results[[4]][4]+vel_rf_results[[4]][5]+
                      vel_rf_results[[4]][6])/3
...
vel_svm_avg_sens <- (vel_svmresults[[4]][1]+vel_svmresults[[4]][2]+
                       vel_svmresults[[4]][3])/3

vel_svm_avg_spec <- (vel_svmresults[[4]][4]+vel_svmresults[[4]][5]+
                       vel_svmresults[[4]][6])/3
...
vel_knn_avg_sens <- (vel_knn_results[[4]][1]+vel_knn_results[[4]][2]+
                       vel_knn_results[[4]][3])/3

vel_knn_avg_spec <- (vel_knn_results[[4]][3]+vel_knn_results[[4]][4]+
                       vel_knn_results[[4]][5])/3
...
accel_gbm_avg_sens <- mean(accel_bl_gbm_results[[4]][1:3])
accel_gbm_avg_spec <- mean(accel_bl_gbm_results[[4]][4:6])
...
accel_rf_avg_sens <- mean(accel_rf_results[[4]][1:3])
accel_rf_avg_spec <- mean(accel_rf_results[[4]][4:6])

accel_svm_accuracy <- accel_svmresults$overall[1]
accel_svm_kappa <- accel_svmresults$overall[2]
accel_svm_AUC <- accel_svmrROC$auc[1]
accel_svm_avg_sens <- mean(accel_svmresults[[4]][1:3])
accel_svm_avg_spec <- mean(accel_svmresults[[4]][4:6])
...
accel_knn_avg_sens <- mean(accel_knn_results[[4]][1:3])
accel_knn_avg_spec <- mean(accel_knn_results[[4]][4:6])
...
abs_accel_gbm_avg_sens <- mean(abs_accel_bl_gbm_results[[4]][1:3])
abs_accel_gbm_avg_spec <- mean(abs_accel_bl_gbm_results[[4]][4:6])
...
abs_accel_rf_avg_sens <- mean(abs_accel_rf_results[[4]][1:3])
abs_accel_rf_avg_spec <- mean(abs_accel_rf_results[[4]][4:6])
...
abs_accel_svm_avg_sens <- mean(abs_accel_svmresults[[4]][1:3])
abs_accel_svm_avg_spec <- mean(abs_accel_svmresults[[4]][4:6])
...
abs_accel_knn_avg_sens <- mean(abs_accel_knn_results[[4]][1:3])
abs_accel_knn_avg_spec <- mean(abs_accel_knn_results[[4]][4:6])
...
perm_base_gbm_avg_sens <- mean(perm_base_bl_gbm_results[[4]][1:3])
perm_base_gbm_avg_spec <- mean(perm_base_bl_gbm_results[[4]][4:6])
...
perm_base_rf_avg_sens <- mean(perm_base_rf_results[[4]][1:3])
perm_base_rf_avg_spec <- mean(perm_base_rf_results[[4]][4:6])
...
perm_base_svm_avg_sens <- mean(perm_base_svmresults[[4]][1:3])
perm_base_svm_avg_spec <- mean(perm_base_svmresults[[4]][4:6])
...
perm_base_knn_avg_sens <- mean(perm_base_knn_results[[4]][1:3])
perm_base_knn_avg_spec <- mean(perm_base_knn_results[[4]][4:6])
...
perm_vel_gbm_avg_sens <- mean(perm_vel_bl_gbm_results[[4]][1:3])
perm_vel_gbm_avg_spec <- mean(perm_vel_bl_gbm_results[[4]][4:6])
...
perm_vel_rf_avg_sens <- mean(perm_vel_rf_results[[4]][1:3])
perm_vel_rf_avg_spec <- mean(perm_vel_rf_results[[4]][4:6])
...
perm_vel_svm_avg_sens <- mean(perm_vel_svmresults[[4]][1:3])
perm_vel_svm_avg_spec <- mean(perm_vel_svmresults[[4]][4:6])
...
perm_vel_knn_avg_sens <- mean(perm_vel_knn_results[[4]][1:3])
perm_vel_knn_avg_spec <- mean(perm_vel_knn_results[[4]][4:6])
...
perm_accel_gbm_avg_sens <- mean(perm_accel_bl_gbm_results[[4]][1:3])
perm_accel_gbm_avg_spec <- mean(perm_accel_bl_gbm_results[[4]][4:6])
...
perm_accel_rf_avg_sens <- mean(perm_accel_rf_results[[4]][1:3])
perm_accel_rf_avg_spec <- mean(perm_accel_rf_results[[4]][4:6])
...
perm_accel_svm_avg_sens <- mean(perm_accel_svmresults[[4]][1:3])
perm_accel_svm_avg_spec <- mean(perm_accel_svmresults[[4]][4:6])
...
perm_accel_knn_avg_sens <- mean(perm_accel_knn_results[[4]][1:3])
perm_accel_knn_avg_spec <- mean(perm_accel_knn_results[[4]][4:6])
...
perm_abs_accel_gbm_avg_sens <- mean(perm_abs_accel_bl_gbm_results[[4]][1:3])
perm_abs_accel_gbm_avg_spec <- mean(perm_abs_accel_bl_gbm_results[[4]][4:6])
...
perm_abs_accel_rf_avg_sens <- mean(perm_abs_accel_rf_results[[4]][1:3])
perm_abs_accel_rf_avg_spec <- mean(perm_abs_accel_rf_results[[4]][4:6])
...
perm_abs_accel_svm_avg_sens <- mean(perm_abs_accel_svmresults[[4]][1:3])
perm_abs_accel_svm_avg_spec <- mean(perm_abs_accel_svmresults[[4]][4:6])
...
perm_abs_accel_knn_avg_sens <- mean(perm_abs_accel_knn_results[[4]][1:3])
perm_abs_accel_knn_avg_spec <- mean(perm_abs_accel_knn_results[[4]][4:6])
